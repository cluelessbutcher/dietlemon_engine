...
void input_update(double delta_time) {
  ...
  memcpy(&state_ptr->keyboard_previous, &state_ptr->keyboard_current, sizeof(keyboard_state));
  memcpy(&state_ptr->mouse_previous, &state_ptr->mouse_current, sizeof(mouse_state));
}
